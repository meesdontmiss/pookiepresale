...
function copyFixedScripts() {
  console.log('\nSetting up scripts with the correct table structure...');
  
  // Update treasury-functions.sql with the final version that creates all tables
  if (fs.existsSync('./scripts/treasury-functions-final.sql')) {
    try {
      fs.copyFileSync('./scripts/treasury-functions-final.sql', './scripts/treasury-functions.sql');
      console.log('✅ Copied treasury-functions-final.sql to treasury-functions.sql');
    } catch (error) {
      console.error('❌ Error copying treasury-functions-final.sql:', error.message);
    }
  } else {
    console.log('⚠️ treasury-functions-final.sql not found, skipping');
  }
  
  // Copy other fixed scripts
  const scripts = [
...
const sqlInstructions = () => {
  ...
  console.log('IMPORTANT: The SQL has been updated to create ALL necessary tables and views!');
  console.log('1. Go to your Supabase dashboard (https://kojysqbrqsfwqvabpkvh.supabase.co)');
...
const finishSetup = () => {
  console.log('\n=== Setup Complete ===');
  console.log('\nIMPORTANT: Before proceeding, you must run the SQL script in your Supabase dashboard!');
  console.log('Otherwise, the scripts will not work correctly.');
  
  rl.question('\nHave you run the SQL script in your Supabase dashboard? (y/n) ', (answer) => {
    if (answer.toLowerCase() !== 'y') {
      console.log('\n⚠️ Please run the SQL script first before using any of the scripts.');
      console.log('You can find it in scripts/treasury-functions.sql');
      rl.close();
      return;
    }

...
// Configuration
const BATCH_SIZE = 10; // Process 10 transactions at a time to avoid rate limits
const MIN_SOL_AMOUNT = 0.001; // Minimum amount to consider a valid contribution
const SLEEP_BETWEEN_REQUESTS = 5000; // 5 seconds between API requests to avoid rate limits
const SLEEP_BETWEEN_BATCHES = 15000; // 15 seconds between batches
...
async function getRecentTransactionSignatures(limit = 20) {
  try {
...
async function processTransaction(signature) {
  try {
    ...
    const existsResponse = await fetch(
      `${supabaseUrl}/rest/v1/contributions?transaction_signature=eq.${signature}&select=id`,
      {
        method: 'GET',
        headers: {
          'apikey': supabaseKey,
    ...
    if (!existsResponse.ok) {
      console.error(`${colors.red}Error checking if transaction exists:${colors.reset}`, await existsResponse.text());
      return null;
    }
    
    const existingTx = await existsResponse.json();
...
async function processBatch(signatures, batchNumber, totalBatches) {
  ...
  for (const sigInfo of signatures) {
    const contribution = await processTransaction(sigInfo.signature);
...
async function main() {
  try {
    console.log(`${colors.bright}${colors.magenta}=== POOKIE Transaction Monitor ===\n${colors.reset}`);
    ...
    // Get recent transactions (reduced limit to avoid rate limits)
    const signatures = await getRecentTransactionSignatures(20); // Start with just 20 transactions
    ...
    console.log(`${colors.cyan}Will process ${signatures.length} transactions in ${batches.length} batches of ${BATCH_SIZE}${colors.reset}`);
    ...
    for (let i = 0; i < batches.length; i++) {
      const { successCount, totalAmount } = await processBatch(batches[i], i + 1, batches.length);
    ...
    console.log(`${colors.green}Successfully imported ${totalSuccessCount} contributions${colors.reset}`);
...
main()
  .then(() => {
    console.log(`\n${colors.bright}${colors.green}Done! Transaction monitoring complete.${colors.reset}`);
    process.exit(0);
  })
  .catch(err => {

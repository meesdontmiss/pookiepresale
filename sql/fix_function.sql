-- Fix the ambiguous function issue by explicitly listing all possible signatures

-- List current functions to verify what we're dealing with
SELECT pg_get_functiondef(oid) 
FROM pg_proc 
WHERE proname = 'calculate_token_allocations' 
AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public');

-- Drop all potential variations of the function
DROP FUNCTION IF EXISTS public.calculate_token_allocations(numeric);
DROP FUNCTION IF EXISTS public.calculate_token_allocations(numeric, numeric);
DROP FUNCTION IF EXISTS public.calculate_token_allocations(numeric, numeric, numeric);

-- Now create our updated function with the correct parameters
...
GRANT EXECUTE ON FUNCTION public.calculate_token_allocations(numeric, numeric, numeric) TO pookie_admin; 